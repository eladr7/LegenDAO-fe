...
export default function RoadmapArticle(): React.ReactElement {
  ...
  const articleChildren = renderArr();
      ...
      >
        ...
        <div
          className={cn(
            ...
            "bg-no-repeat bg-cover bg-bottom -z-20"
          )}
        ...
        <div
          className={cn(
            "absolute top-0 left-0 right-0 h-[200px] -z-10",
            "bg-gradient-to-b from-slate-900 to-slate-900/0"
          )}
        ></div>
        <ArticleBox
          className="bg-contain bg-repeat-round justify-between pl-48 pr-36 pb-4"
          childElement={articleChildren}
        />
      </div>
    </Article>
  );
